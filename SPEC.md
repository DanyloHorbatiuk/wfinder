...
Хеш — sha256 (hex) від канонічного JSON відстежуваних полів: `title, url, course_type, direction, format, level, price, date_start, date_end, status, country, city, languages`, а після I-01 також `description` (`tags` не додається — I-01 не знайшов такого поля в жодному джерелі, див. §15 Q1).
...
- **`fetch_one_source`** піднімає виняток при будь-якій помилці: HTTP, невалідний JSON (перевіряти `json.loads` до збереження), MinIO, БД. Параметри `retries=3` і `retry_delay=2 хв` лишаються. Час знімка `fetched_at = utcnow()`; ключ `raw/{source}_{YYYYMMDDTHHMMSSZ}.json` у справжньому UTC.
  - **SoftServe:** ендпоінт пагінований (`meta.total`, `meta.last_page`, `meta.per_page`, `links.next` — підтверджено I-01, §15 Q2). Поточний запит без `page` бере лише першу сторінку. Потрібно пройти сторінки `1..meta.last_page` і об'єднати `data[]` в один знімок **до** збереження в MinIO, інакше знімок буде неповним і захист E-06 спрацьовуватиме помилково. У наданому I-01 знімку `last_page=1`, тому дефект поки не проявляється на практиці, але обробку пагінації потрібно реалізувати в F-02, а не покладатися на поточний розмір відповіді.
...
|---|---|---|---|
| Q1 | Які текстові поля (опис, теги, навички) є в сирому JSON кожного джерела? | E-01, F-02 (поля хешу) | EPAM (`pageProps.trainings.Items`, 206 записів у знімку): лише `Name` (заголовок, ≤ 62 символи в знімку). Полів `Description`/`Tags` немає; `AdditionalSkillsIds`/`AdditionalSkillsLogos` є в схемі, але порожні в усіх 206 записах — ненадійні. SoftServe (`data[]`): є `description` (реальний текст курсу, ВЕЛИКИМИ ЛІТЕРАМИ), `name`, `type_name`; поля `tags` немає. Підсумок: `tags` не додаємо в жодне джерело; `description` — лише для SoftServe (SPEC §4.2, §5.2 оновлено) |
| Q2 | Чи містить кожен файл повний список пропозицій джерела, без пагінації? Порівняти кількість елементів із полем загальної кількості, якщо воно є. | E-06 | EPAM: жодного поля пагінації/лічильника в `pageProps.trainings` не знайдено; URL знімка без параметра сторінки → ймовірно повний список (206), але математично підтвердити неможливо без лічильника. SoftServe: **є пагінація** — `meta.total`, `meta.last_page`, `meta.per_page=20`, `links.next`; у наданому знімку `total=1, last_page=1`, тобто зараз усе на одній сторінці, але це не гарантовано. Поточний фетч бере лише сторінку 1. **Рішення:** F-02 повинен пройти `1..meta.last_page` для SoftServe перед збереженням знімка (додано в ROADMAP F-02 і SPEC §6.1) |
| Q3 | Чи стабільний URL EPAM? Якщо в ньому є buildId Next.js (`_next/data/<id>/...`), він змінюється після кожного деплою сайту. | F-03, E-07 | Підтверджено: URL містить `buildId` (`yQvZROZOVSjx62IVxVebj`), який зміниться після наступного деплою сайту EPAM (Next.js повертає 404 для застарілого buildId). Це не помилка пайплайна: після F-03 `fetch_one_source[epam]` впаде з чіткою помилкою після 3 спроб, сповіщення (E-07) покаже збій, а `CAREERS_URL_EPAM` доведеться оновлювати вручну — задокументоване обмеження |
| Q4 | Скільки файлів у `raw/` і за який період? Чи є пропуски днів? | E-03, E-02 | Не застосовується зараз: MinIO в цьому середовищі порожній, пайплайн ще не працював на реальному стеку. Відповідь можлива лише після кількох тижнів реальної роботи DAG — уточнити перед стартом E-03/E-02 |
| Q5 | Які значення має `status` у кожному джерелі й що вони означають (реєстрацію відкрито чи закрито)? | E-02 (можлива друга метрика: тривалість відкритої реєстрації) | EPAM `Status` у знімку: лише `RegistrationOpen` (202) і `Planned` (4); значення на кшталт "закрито" немає — закриті тренінги, судячи з усього, просто зникають зі списку (підтверджує D3). SoftServe: у знімку лише 1 запис (`"Open for Registration"`), вибірка замала для повного переліку; за коментарем у `core/model.py` очікується принаймні цей варіант. Повний діапазон статусів SoftServe лишається відкритим, не блокує F-02 |
| Q6 | Фільтр EPAM (Україна, `PlanLevel > 2`): якщо пропозиція перестає під нього підпадати, вона вважається закритою. Чи прийнятно це? | E-06, E-02 | У знімку: 206 записів усього, 20 по Україні, 9 проходять чинний фільтр (`PlanLevel > 2`). Фільтр уже реалізований у `EpamAdapter.parse()`. **Рішення:** залишити поведінку без змін — запис, що перестав проходити фільтр, зникає зі знімка адаптера і закриється через звичайну логіку E-06 (`close_reason='removed'`); окремого стану "не пройшов фільтр" не вводимо, §5.3 п.5 ("відхилені записи") стосується лише E-05-карантину невалідних даних, а не бізнес-фільтра адаптера |
